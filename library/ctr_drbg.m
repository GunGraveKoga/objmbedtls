...
int mbedtls_ctr_drbg_write_seed_file( mbedtls_ctr_drbg_context *ctx, const char *path )
{
    ...
    @try {
      path_ = [[OFString alloc] initWithUTF8String:path];
      f = [[OFFile alloc] initWithPath:path_ mode:@"wd"];
    }@catch(OFException* e) {
      [path_ release];
      return( MBEDTLS_ERR_CTR_DRBG_FILE_IO_ERROR );
    }
    ...
    @try {
      [f writeBuffer:buf length:MBEDTLS_CTR_DRBG_MAX_INPUT];
    }@catch(OFException* e) {
...
int mbedtls_ctr_drbg_update_seed_file( mbedtls_ctr_drbg_context *ctx, const char *path )
{
    ...
    //n = (size_t) ftell( f );
    //fseek( f, 0, SEEK_SET );
    [f seekToOffset:0 whence:SEEK_END];
    n = [f seekToOffset:0 whence:SEEK_CUR];
    [f seekToOffset:0 whence:SEEK_SET];
