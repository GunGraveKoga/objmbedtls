...
cleanup:
    ...
    OFString* path_ = [OFString stringWithUTF8String:path];
    OFFileManager* manager = [OFFileManager defaultManager];

    if (![manager directoryExistsAtPath:path_]) {
        objc_autoreleasePoolPop(pool);
        return( MBEDTLS_ERR_X509_BAD_INPUT_DATA );
    }
    @try {
        for (OFString* item in [manager contentsOfDirectoryAtPath:path_]) {
            void* pool_ = objc_autoreleasePoolPush();

            OFString* itemPath = [[path_ stringByAppendingPathComponent:item] stringByStandardizingPath];
            if ([manager directoryExistsAtPath:itemPath]) {
                objc_autoreleasePoolPop(pool_);
                continue;
            }

            w_ret = mbedtls_x509_crt_parse_file( chain, [itemPath UTF8String]);
